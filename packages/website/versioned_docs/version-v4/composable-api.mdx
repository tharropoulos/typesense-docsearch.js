...
- `typesense-docsearch-core` - Shared core logic for managing different states of DocSearch
- `typesense-docsearch-modal` - The actual components used for the DocSearch Modal

:::warning
Because of the nature of composability, this API is only available within React, and not within the `typesense-docsearch.js` package.
:::
...
```bash
npm install typesense-docsearch-core typesense-docsearch-modal typesense-docsearch-css
```
...
```bash
yarn add typesense-docsearch-core typesense-docsearch-modal typesense-docsearch-css
```
...
```bash
pnpm add typesense-docsearch-core typesense-docsearch-modal typesense-docsearch-css
```
...
```bash
bun add typesense-docsearch-core typesense-docsearch-modal typesense-docsearch-css
```
...
import 'typesense-docsearch-css/style.css';
...
To help aid in trimming initial bundle size, the `typesense-docsearch-modal` package exposes explicit file exports as well:
...
async function importDocSearchModalIfNeeded() {
  ...
  const { DocSearchModal: Modal } = await import('typesense-docsearch-modal/modal');
...
The `<DocSearch />` component from the `typesense-docsearch-core` package is the main state handler for all of DocSearch.
