...
description: Configure typesense-docsearch.js 5 and control its returned instance.
...
The default export from `typesense-docsearch.js` is AI-capable and uses `DocSearchAI`. The default export from `typesense-docsearch.js/docsearch` is keyword-only and uses `DocSearch`.
...
Ask AI is available from the default `typesense-docsearch.js` entry. Its `askAi` option is required by that entry's `DocSearchProps` type.
