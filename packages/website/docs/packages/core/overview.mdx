---
title: Core package
description: Understand the shared state and controls in typesense-docsearch-core v5.
---

import TabItem from '@theme/TabItem';
import Tabs from '@theme/Tabs';

`typesense-docsearch-core` provides the React state shared by DocSearch views. It owns open and close state, keyboard events, theme selection, lifecycle callbacks, initial queries, and imperative controls. It doesn't render a search interface by itself.
...
```bash
npm install typesense-docsearch-core@^5
```
...
```bash
yarn add typesense-docsearch-core@^5
```
...
```bash
pnpm add typesense-docsearch-core@^5
```
...
```bash
bun add typesense-docsearch-core@^5
```

  </TabItem>
</Tabs>

Packages such as `typesense-docsearch-modal` and `typesense-docsearch-sidepanel` consume this provider. For component composition instructions, see the [Composable API guide](/docs/composable-api).
