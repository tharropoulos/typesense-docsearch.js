...
description: API reference for typesense-docsearch-sidepanel.
toc_max_heading_level: 2
---

## `Sidepanel`

Render `Sidepanel` inside `DocSearch` from `typesense-docsearch-core`.
...
`InitialAskAiMessage` is `{ query: string; messageId?: string; suggestedQuestionId?: string }`. See the [`typesense-docsearch-core` API](../core/api) for the full ref.

## Exports

| Import path | Value exports | Type exports |
| --- | --- | --- |
| `typesense-docsearch-sidepanel` | `Sidepanel`, `SidepanelButton` | `SidepanelProps`, `SidepanelButtonProps` |
| `typesense-docsearch-sidepanel/sidepanel` | `Sidepanel` | `SidepanelProps` |
| `typesense-docsearch-sidepanel/button` | `SidepanelButton` | `SidepanelButtonProps` |
