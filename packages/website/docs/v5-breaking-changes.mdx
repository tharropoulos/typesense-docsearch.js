...
In v4, the root `typesense-docsearch.js` export rendered the combined component and allowed Ask AI to be omitted. In v5, it renders `DocSearchAI`, and its `DocSearchProps` type requires `askAi`.
...
`typesense-docsearch.js` now exports only `.` and `./docsearch`. Replace imports of internal distribution files with one of these public entry points. Direct CDN URLs to the two documented UMD files remain supported by the package layout.
...
The package also adds `typesense-docsearch-react/docsearchAi` and `typesense-docsearch-react/askaiModal` subpaths.
...
`typesense-docsearch-modal` exports the AI modal from its root and from `typesense-docsearch-modal/askai`.
...
The complete `typesense-docsearch-css` stylesheet still imports button, modal, and Ask AI rules. React also exposes split style entries:

- `typesense-docsearch-react/style/variables`
- `typesense-docsearch-react/style/button`
- `typesense-docsearch-react/style/modal`
- `typesense-docsearch-react/style/askai`
- `typesense-docsearch-react/style/sidepanel`
...
`typesense-docsearch-react` changes `main` from `dist/umd/index.js` to `dist/esm/index.js`. Consumers that resolve `main` instead of the package exports need an ESM-compatible build pipeline. The explicit `unpkg` and `jsdelivr` fields continue to point to `dist/umd/index.js`.
...
`typesense-docsearch-react`, `typesense-docsearch-core`, `typesense-docsearch-modal`, and `typesense-docsearch-sidepanel` declare these optional peers:

- `react`: `>=16.8.0 <20.0.0`
- `react-dom`: `>=16.8.0 <20.0.0`
- `@types/react`: `>=16.8.0 <20.0.0`

`typesense-docsearch-react` also accepts optional `search-insights` versions `>=1 <3`.
...
Install `typesense-docsearch-css@^5`, then import `typesense-docsearch-css`. For a CDN integration, load `typesense-docsearch-css/dist/style.css`.
