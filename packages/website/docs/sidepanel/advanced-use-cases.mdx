...
available from `typesense-docsearch-sidepanel`. The `typesense-docsearch-sidepanel` package can be installed as follows:

<Tabs>
<TabItem value="npm">

```bash
npm install typesense-docsearch-sidepanel
```
...
```bash
yarn add typesense-docsearch-sidepanel
```
...
```bash
pnpm add typesense-docsearch-sidepanel
```
...
```bash
bun add typesense-docsearch-sidepanel
```
...
async function importSidepanelIfNeeded() {
  ...
  const { Sidepanel: Panel } = await import('typesense-docsearch-sidepanel/sidepanel');
