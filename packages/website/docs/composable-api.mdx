...
- [`typesense-docsearch-core`](/docs/packages/core/overview) provides `DocSearch`, shared state, keyboard handling, and the imperative ref.
- [`typesense-docsearch-modal`](/docs/packages/modal/overview) provides the provider-connected button and modal components.
- [`typesense-docsearch-css`](/docs/packages/css/styling) provides the styles.
...
```tsx
import { DocSearch, type DocSearchRef } from 'typesense-docsearch-core';
import { DocSearchAskAiModal, DocSearchButton } from 'typesense-docsearch-modal';
...
import type { DocSearchAskAiModalProps } from 'typesense-docsearch-modal/askai';
import { lazy, Suspense, type JSX } from 'react';

import 'typesense-docsearch-css/dist/style.css';

let modalImport: Promise<typeof import('typesense-docsearch-modal/askai')> | undefined;

function loadModal(): Promise<typeof import('typesense-docsearch-modal/askai')> {
  modalImport ??= import('typesense-docsearch-modal/askai');
  return modalImport;
}
...
| Import | Exports |
| --- | --- |
| `typesense-docsearch-core` | `DocSearch`, `useDocSearch`, their types, keyboard utilities, and theme utilities |
| `typesense-docsearch-modal` | `DocSearchButton`, `DocSearchModal`, `DocSearchAskAiModal`, and their prop types |
| `typesense-docsearch-modal/button` | `DocSearchButton`, `DocSearchButtonProps` |
| `typesense-docsearch-modal/modal` | `DocSearchModal`, `DocSearchModalProps` |
| `typesense-docsearch-modal/askai` | `DocSearchAskAiModal`, `DocSearchAskAiModalProps` |

The lower-level React entry points are `typesense-docsearch-react/button`, `typesense-docsearch-react/modal`, and `typesense-docsearch-react/askaiModal`. They don't connect themselves to the composable provider. Use them only when you intend to manage portal rendering, refs, `initialScrollY`, close behavior, and Ask AI state yourself.
...
```ts
import 'typesense-docsearch-css/dist/_variables.css';
import 'typesense-docsearch-css/dist/button.css';
import 'typesense-docsearch-css/dist/modal.css';
```

Add `typesense-docsearch-css/dist/_askai.css` when you use `DocSearchAskAiModal`. Bundlers can also load the same files through `typesense-docsearch-react/style`, `typesense-docsearch-react/style/button`, `typesense-docsearch-react/style/modal`, `typesense-docsearch-react/style/askai`, and `typesense-docsearch-react/style/variables`.
